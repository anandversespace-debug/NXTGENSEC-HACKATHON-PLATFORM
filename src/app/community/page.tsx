...
const CommunityPage = () => {
  ...
  const categories = [
    ...
    { id: 'judge', label: 'Auditors' },
    { id: 'admin', label: 'Core Team' },
  ];
  ...
  if (loading) {
    return (
      <div className="min-h-screen pt-24 pb-20 px-6 bg-[#050505] flex items-center justify-center">
        <div className="text-blue-500 font-black italic animate-pulse tracking-widest uppercase text-xs">Accessing Collaboration Protocol Registry...</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen pt-24 pb-20 px-6 bg-[#050505] overflow-hidden">
      ...
      <div className="fixed top-0 left-0 w-full h-full pointer-events-none opacity-[0.02] fixed inset-0 bg-[linear-gradient(to_right,#80808012_1px,transparent_1px),linear-gradient(to_bottom,#80808012_1px,transparent_1px)] bg-[size:40px_40px]"></div>
      ...
      <div className="max-w-7xl mx-auto space-y-16 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div className="max-w-2xl">
             <div className="inline-flex items-center space-x-2 bg-blue-600/10 border border-blue-600/20 rounded-full px-4 py-1.5 mb-6">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-400">Collaboration Protocol</span>
             </div>
             <h1 className="text-4xl md:text-5xl font-black italic uppercase tracking-tighter text-white mb-4">
                Global Developer <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-500 italic">Node Hub</span>
             </h1>
             <p className="text-gray-500 text-sm font-bold uppercase tracking-tighter leading-relaxed max-w-xl">
                Identify and connect with the ecosystem's elite cryptographic researchers, security auditors, and system architects. Every node is verified and ranked by their contribution hash.
             </p>
          </div>
          <div className="flex bg-[#0c0c0c] border border-white/5 rounded-xl p-6 md:p-8 space-x-12 shrink-0">
             <div className="text-center">
                <p className="text-[9px] font-bold text-gray-700 uppercase tracking-widest mb-1 font-black">Total Entites</p>
                <p className="text-2xl font-black text-white italic tracking-tighter">{developers.length}</p>
             </div>
             <div className="w-px h-full bg-white/5" />
             <div className="text-center">
                <p className="text-[9px] font-bold text-gray-700 uppercase tracking-widest mb-1 font-black">Active Backbone</p>
                <p className="text-2xl font-black text-blue-500 italic tracking-tighter">ONLINE</p>
             </div>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-6 items-center justify-between">
           ...
           <div className="w-full md:w-96 relative group">
              ...
              <input 
                type="text" 
                placeholder="Lookup Entity by Name or Handle..." 
                value={searchQuery}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pb-20 text-white">
           {filteredDevelopers.map((dev, idx) => (
             ...
             >
                ...
                <p className="text-[10px] text-gray-500 leading-relaxed font-bold uppercase tracking-tighter h-10 mb-8 border-l-2 border-white/5 pl-4 group-hover:border-blue-500/30 transition-colors line-clamp-2">
                  {dev.bio || 'Entity has not established a public intelligence profile.'}
                </p>

                <div className="flex flex-wrap gap-2 mb-8 h-[52px] content-start overflow-hidden">
                   {(dev.skills || ['Core Node']).map((skill: string, i: number) => (
                ...
                <div className="grid grid-cols-2 gap-4 pt-6 border-t border-white/[0.03]">
                   <div>
                      <p className="text-[8px] text-gray-700 font-bold uppercase tracking-widest mb-1.5 italic">Contribution Hash</p>
                      <div className="flex items-center space-x-2">
                        <Activity className="w-3.5 h-3.5 text-blue-500" />
                        <span className="text-xs font-black text-gray-300 italic">ID_{dev._id.slice(-4).toUpperCase()}</span>
                      </div>
                   </div>
                   <div>
                      <p className="text-[8px] text-gray-700 font-bold uppercase tracking-widest mb-1.5 italic">Reputation CP</p>
                      <div className="flex items-center space-x-2">
           ...
           {filteredDevelopers.length === 0 && (
             <div className="col-span-full py-20 text-center border border-dashed border-white/5 rounded-2xl">
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest text-gray-600 italic">No node identities found matching specified filter parameters.</p>
             </div>
           )}
        </div>
